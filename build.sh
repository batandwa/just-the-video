...
ssh -o StrictHostKeyChecking=no -i /tmp/id_rsa $APP_HOST_USER@$APP_HOST cd $DEST_DIR && pwd && TAG=$TAG docker-compose pull web && TAG=$TAG docker-compose up -d --force-recreate
