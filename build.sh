...
ssh -o StrictHostKeyChecking=no -i /tmp/id_rsa $APP_HOST_USER@$APP_HOST ls -alh /
scp -o StrictHostKeyChecking=no -i /tmp/id_rsa docker-compose.yml $APP_HOST:$DEST_DIR
ssh -o StrictHostKeyChecking=no -i /tmp/id_rsa $APP_HOST cd $DEST_DIR && docker-compose up -d
